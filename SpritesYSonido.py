...
rotación = 0  # Ángulo inicial del jugador
imagenComida = pygame.image.load('cereza.png')
imagenComida = pygame.transform.scale(imagenComida, (20, 20))
comidas = []
for i in range(20):
...
while True:
    ...
    # pintar el fondo sobre la superficie
    superficieVentana.blit(imagenFondo, (0, 0))
    ...
    for comida in comidas[:]:
        if jugador.colliderect(comida):
            ...
            rotación -= 10  # Rotar 10 grados a la izquierda
            if músicaSonando:
                sonidoRecolección.play()

    # dibujar al jugador con rotación
    imagenRotadaJugador = pygame.transform.rotate(imagenEstiradaJugador, rotación)
    jugadorCentro = (jugador.centerx - imagenRotadaJugador.get_width() // 2,
                     jugador.centery - imagenRotadaJugador.get_height() // 2)
    superficieVentana.blit(imagenRotadaJugador, jugadorCentro)
